...
static int extract_string(const char *src, char *dest, int len)
{
    ...
    for (pos = 0; (pos < (int)strlen(src)) && (pos < len); pos++) {
        if ((off == -1) && (src[pos] == '"')) {
            off = pos + 1;
        } else if (off >= 0) {
            if (esc == 1) {
    ...
    return (off > -1) ? (int)strlen(dest) : -1;
}
...
static void print_msg(struct msg *msg, FILE *out)
{
    ...
    if (msg->id && msg->val[0]) {
        for (i = 0; i <= msg->plural_num; i++) {
            ...
            if (msg->ctxt && msg->id_plural)
                snprintf(key, sizeof(key), "%s\1%s\2%d",
                         msg->ctxt, msg->id, i);
            else if (msg->ctxt)
                snprintf(key, sizeof(key), "%s\1%s",
                         msg->ctxt, msg->id);
            else if (msg->id_plural)
                snprintf(key, sizeof(key), "%s\2%d",
                         msg->id, i);
            else
            ...
            if (key_id != val_id) {
                ...
                for (size_t j = entry->length; j < len; j++)
                    fputc(0, out);
    ...
    for (i = 0; i < (int)(sizeof(msg->val) / sizeof(msg->val[0])); i++)
        free(msg->val[i]);
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        if (!strncmp(line, "msgctxt \"", 9)) {
            ...
            msg.len       = 0;
        } else if (!strncmp(line, "msgstr \"", 8) ||
                   !strncmp(line, "msgstr[", 7)) {
            if (line[6] == '[')

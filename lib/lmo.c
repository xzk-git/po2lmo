                        ...
                        const char *ctx, int ctxlen, int plural)
{
    ...
    if (ctx) {
        ...
        if ((ptr > res) && isspace((unsigned char)*(ptr - 1)))
            ptr--;
...
lmo_archive_t *lmo_open(const char *file)
{
    ...
    if ((ar = (lmo_archive_t *)malloc(sizeof(*ar))) != NULL) {
        ...
        /* POSIX: use mmap */
        ar->mmap = mmap(NULL, ar->size, PROT_READ, MAP_SHARED, ar->fd, 0);
        if (ar->mmap == MAP_FAILED)
            goto err;
#else
        /* Windows: malloc + fread */
        {
            FILE    *fp  = fdopen(ar->fd, "rb");
            uint8_t *buf = NULL;

            if (!fp)
                goto err;

            buf = (uint8_t *)malloc(ar->size);
            if (!buf) {
                fclose(fp);
                goto err;
            }

            if (fread(buf, 1, ar->size, fp) != (size_t)ar->size) {
                free(buf);
                fclose(fp);
                goto err;
            }
...
/* plural parser interface from plural_formula.c */
void *pluralParseAlloc(void *(*mallocProc)(size_t));
void  pluralParse(void *yyp, int yymajor, int yyminor, void *plur);
void  pluralParseFree(void *p, void (*freeProc)(void *));
...
int lmo_translate_plural(int n,
                         const char *skey, int skeylen,
                         const char *pkey, int pkeylen,
                         char **out, int *outlen)
{
    return lmo_translate_plural_ctxt(n, skey, skeylen,
                                     pkey, pkeylen,
                                     NULL, 0, out, outlen);
}

int lmo_translate_plural_ctxt(int n,
                              const char *skey, int skeylen,
                              const char *pkey, int pkeylen,
                              const char *ctx, int ctxlen,
                              char **out, int *outlen)
{
    ...
    for (ar = _lmo_active_catalog->archives; ar; ar = ar->next) {
        ...
        if (e != NULL) {
            pid = lmo_eval_plural(ar->mmap + ntohl(e->offset),
                                  ntohl(e->length), n);
            break;
        }
    }
...
void lmo_iterate(lmo_iterate_cb_t cb, void *priv)
{
    ...
    for (ar = _lmo_active_catalog->archives; ar; ar = ar->next) {
        for (i = 0, e = &ar->index[0]; i < (unsigned int)ar->length; e = &ar->index[++i]) {
            cb(ntohl(e->key_id),
               ar->mmap + ntohl(e->offset),
               ntohl(e->length),
               priv);
        }
    }
